...
面向含能材料研究的全栈 AI 系统，核心是一个**总控智能体（Zongkong Orchestrator）**：根据用户意图自动分派到 6 类专业子 Agent——数据查询、实验设计、文献检索、混合链式、方案修改、安全评估。每个子 Agent 均支持流式 SSE 输出，前端实时展示 Agent 徽章、工具调用计划和分段耗时。底层依托混合检索、知识图谱、MCP 工具集成，并配备基于实验员视角的 19 题细粒度评估套件（239 分，当前得分 ~96%）。

前端为 **PyroPlanner**——一个基于 Next.js 的对话式界面，支持流式输出、Agent 路由可视化、对话历史管理和多轮上下文连续对话。
...
**对话中**——用户消息蓝色渐变气泡，助理回答白色卡片（含 Agent 路由徽章、工具调用计划标签、Markdown 表格渲染、底部响应耗时分段显示），侧栏激活项蓝色渐变指示：
...
根问题（实验闭环：目标→设计→安全→实验→分析→迭代）
   ...
   │
   └─► 第九层：闭环评估
         19题239分 · 化学硬指标自动核验 · 否定语境感知 · --workers并行 · 53%→96%
```
...
总控智能体（Zongkong Orchestrator）是系统的入口层，通过一次 LLM 调用对用户查询进行意图分类，然后将请求路由到最合适的专业子 Agent。

### 意图分类与路由

系统支持 6 种意图类型，每种意图对应一个专业 Agent：

| 意图 | Agent | 图标 | 触发条件 | 示例 |
|------|-------|------|---------|------|
| `data` | 数据查询 Agent | 📊 | 1-3 个明确物质 + 单一数值属性 + 答案是数值 | "RDX 的密度是多少" |
| `experiment` | 实验设计 Agent | 🔬 | 请求设计配方/方案/步骤，或优化现有配方性能 | "设计一个燃速 > 15mm/s 的低感度固体推进剂" |
| `literature` | 文献检索 Agent | 📚 | 机理分析、影响因素、综述、知识问答（默认路径） | "AP 粒径对燃速有什么影响" |
| `hybrid` | 数据+实验链式 Agent | ⚡ | 明确要求先查数值、再基于数值设计配方（链式执行） | "帮我找 RDX 的密度，再基于这个设计一个配方" |
| `modify` | 方案修改 Agent | ✏️ | 明确指向上一次实验/配方方案，要求调整参数（最高优先级） | "调整一下配方，把铝粉比例降到 15%" |
| `safety` | 安全评估 Agent | 🛡️ | 安全风险、相容性、危险性、储存/运输要求 | "AP 和铝粉能混合使用吗？" |

### 意图判断规则

**`data`（数值查询）** — 同时满足：
- 问题涉及 1-3 个**明确命名的物质**
- 问的是**单一具体属性**（密度、爆速、熔点等）
- 答案是数值，不需要机理解释

**`experiment`（实验设计）** — 满足其一：
- 请求设计配方、方案、实验步骤
- 请求优化现有配方某项性能
- 请求给出组分比例建议

**`safety`（安全评估）** — 满足其一（高优先级）：
- 问的是安全风险、危险性、爆炸危险
- 问两种组分能否相容/混合
- 给出具体配方（含百分比），问该配方是否安全

**`modify`（方案修改）** — 最高优先级：
- 用户明确指向上一个实验/配方方案，要求调整其中某个参数
- 关键词：调整、修改、改成、在上个方案基础上

**`hybrid`（链式执行）** — 同时满足：
- 明确要求先查某物质的某个数值
- 然后基于该数值或物质设计配方/实验

**`literature`（文献检索）** — 默认路径：
- 所有不满足上述条件的查询：机理、综述、多属性混合问题

### SSE 事件序列

```
{"type": "start"}
{"type": "progress", "phase": "routing", "message": "总控正在分析请求类型…"}
{"type": "route", "agent": "文献检索 Agent", "icon": "📚", "intent": "literature", "reason": "AP 粒径对燃速机理 → 文献检索路径"}
{"type": "progress", "phase": "routing", "message": "📚 已路由至文献检索 Agent"}
{"type": "plan", "plan": [...], "tool_names": [...]}
{"type": "progress", "phase": "retrieving", "message": "正在检索文献数据…"}
{"type": "token", "content": "AP 粒径对燃速的影响主要体现在…"}
{"type": "done", "elapsed_secs": 12}
```

| 事件 | 触发时机 |
|------|---------|
| `start` | 请求到达后立即发出 |
| `progress` (routing) | 总控开始分类时，以及路由完成时 |
| `route` | 意图分类完成后，携带 agent/icon/intent/reason，前端据此显示彩色 Agent 徽章 |
| `plan` | 子 Agent 生成工具调用计划后 |
| `progress` | 子 Agent 各阶段执行时 |
| `token` | LLM 生成答案时逐 token 推送 |
| `done` | 流结束，携带 `elapsed_secs`（总耗时） |

### Hybrid 链式执行

`hybrid` 意图触发两阶段链式执行：

1. **Phase 1：DataAgent** — 流式输出数据查询结果，收集所有 token；其 `done` 事件被总控截获、抑制（不传递给前端）
2. **分隔符**：总控发出 `progress` 事件（"🔬 阶段二：基于以上数据进行实验设计…"）
3. **Phase 2：ExperimentAgent** — 以原始问题 + DataAgent 输出摘要作为增强查询，运行完整实验设计流程

### 子 Agent 说明

#### DataAgent（数据查询）

- **单实体**：`get_entity_card` 精确拉取实体属性卡
- **多实体**：`ThreadPoolExecutor` 并发运行 `get_entity_card × N` + `compare_values`
- **实体模糊匹配**：4 步 ilike 链（精确 canonical → 精确 alias → 模糊 canonical → 模糊 alias）
- 无命中时自动降级到 `search_memory` 语义检索
- LLM 综合生成回答，控制在 400 字以内，重要数值加粗

#### ExperimentAgent（实验设计）

- 文献检索 → 生成 1-3 个候选配方 → 每个配方独立执行 `check_safety`
- 若所有配方均被 `blocked`：自动 retry，排除危险组分后重新设计
- 多候选方案并行生成（`ThreadPoolExecutor`）

#### ModifyAgent（方案修改）

- 从对话历史（history）中提取最近一条 assistant 消息作为"上一次方案"
- 构造增强请求：修改要求 + [上一次实验方案] 注入 ExperimentAgent

#### SafetyAgent（安全评估）

- 正则解析查询中的配方信息（`AP 68%`、`A/B/C=x/y/z` 等格式）
- 并发执行：实体卡查询 + 语义检索 + `check_safety`（如有明确配方）
- LLM 综合生成安全评估报告

### 历史压缩

当对话历史超过 8000 字符时，自动触发 LLM 压缩：
- 将较早的对话轮次压缩为一段摘要（≤300 字）
- 保留最近 6 条消息原文
- 压缩失败时自动降级为尾部截断（保留最近 500 字）
...
| 功能 | 说明 |
|------|------|
| **总控智能体** | 单次 LLM 调用路由到 6 类专业子 Agent，前端实时显示 Agent 徽章和路由原因 |
| **意图分类** | 6 种意图：data / experiment / literature / hybrid / modify / safety，含优先级规则 |
| **数据查询 Agent** | 模糊实体匹配 + 并发实体卡 + compare_values，快速精准数值查询 |
| **实验设计 Agent** | 文献检索 + 多候选配方生成 + 安全门控 + 全被阻断时自动 retry |
| **文献检索 Agent** | 完整 RAG 流水线：8阶段规划 Agent + 混合检索 + 置信度评分 + 内联引用 |
| **Hybrid 链式 Agent** | DataAgent 输出作为上下文，注入 ExperimentAgent，两阶段无缝衔接 |
| **方案修改 Agent** | 从对话历史提取上一次方案，委托 ExperimentAgent 增量修改 |
| **安全评估 Agent** | 正则解析配方 + 并发数据检索 + check_safety 规则引擎，流式输出 |
| **历史压缩** | 对话超 8000 字符时 LLM 摘要压缩，保留最近 6 轮，降低 token 消耗 |
...
| **3 段式流式兜底** | 流式失败时依次尝试：重试流式（3s 后）→ 非流式兜底（分 80 字节块推送），全程不返回错误字符串 |
...
| **PyroPlanner Chat UI** | Next.js 对话界面，含 Agent 徽章、对话历史侧栏、流式状态指示、响应耗时显示、Markdown 渲染 |
| **对话历史（本地存储）** | 所有会话保存在 localStorage，含参考文献；支持并行流式生成（切换对话不中断后台请求）；对话切换时实时查看任意 session 的生成进度 |
| **实验 Agent 评估套件 v2** | 19 题 239 分，实验员视角细粒度评分，底层化学硬指标自动核验，当前 ~96% |
| **总控路由评估** | 23 题路由/数据/实验评估套件（`eval/zongkong_eval.py`），支持 `--mode routing\|data\|experiment\|all` |
| **知识图谱可视化** | D3 force-directed 画布图谱，节点按类型着色，支持缩放/拖拽/点击高亮邻居 |
| **数据抽取** | 上传 PDF / MinerU JSON，即时 LLM NER 提取实体、属性（含置信度 0–1）、表格；跨页表格自动合并，缺失标题从文档自动推断；支持 CSV / JSON 导出 |
| **对话可视化** | 回答中自动解析 Markdown 表格和百分比模式，渲染为饼图 / 柱图 / 雷达图（可切换） |
| **文献来源卡片** | 对话回答附带可折叠来源列表，显示标题、摘要片段和来源类型（实体/块/文档）；点击跳转对应文档/实体页，历史对话重新打开后参考文献保持可见 |
| **实体对比** | 最多选 5 个实体，弹出雷达图 + 属性对照表 |
| **重新生成** | 助理最后一条回答下方提供"重新生成"按钮，重放上一条用户消息 |
...
> **注意**：前端默认请求 `http://localhost:8000`，需确保后端服务已启动。如需修改后端地址，编辑 `frontend/lib/api.ts` 中的 `BASE_URL` 或设置 `NEXT_PUBLIC_API_BASE` 环境变量（`frontend/.env.local`）。后端 CORS 已配置为允许所有 `localhost` 端口，无需手动更新白名单。
...
| 功能 | 说明 |
|------|------|
| **Agent 路由徽章** | 每条回答顶部展示路由到的 Agent 名称（彩色徽章），以及路由原因（副标题），颜色按 intent 区分 |
| **流式状态指示** | 按 `正在理解问题… → 路由中… → 正在检索… → 正在深度推理… → 正在整理证据…` 序列实时反馈进度 |
...
| **新对话** | 点击"新对话"清空显示，后台流继续运行；支持同时发起多个问答（并行流式），结果各自保存到历史 |
| **内置示例** | 首页预设 4 个典型问题，点击即发送 |
| **数据抽取页 `/extract`** | 拖拽上传 PDF/JSON，LLM NER 提取实体属性（置信度着色），表格智能合并+缺标题自动补全，属性分布柱图，CSV/JSON 导出 |
| **实验历史页 `/experiments`** | 折叠式实验列表，懒加载协议方案，配方表/预测属性/步骤/安全状态一览 |
| **知识图谱页 `/graph`** | 列表/可视化双模式，D3 canvas force 图，节点类型着色，缩放拖拽 |
| **统计页 `/stats`** | 柱图/饼图展示检索分布、命中率、图谱节点/边类型 |

### SSE 事件序列

后端 `/chat/stream` 接口以 SSE 格式推送以下事件：
...
| `progress` (routing) | 总控开始分类 / 路由完成时 |
| `route` | 意图分类完成后发出；前端据此显示彩色 Agent 徽章和路由原因 |
| `plan` | 子 Agent 生成工具调用计划后发出；携带 `phase1_secs`（检索阶段耗时） |
| `progress` | 每个 Agent 节点执行时发出 |
...
| intent | 配色 |
|--------|------|
| `literature` | 蓝色（bg-blue-50 / border-blue-200） |
| `experiment` | 紫色（bg-violet-50 / border-violet-200） |
| `data` | 青色（bg-teal-50 / border-teal-200） |
| `hybrid` | 橙色（bg-orange-50 / border-orange-200） |
| `modify` | 琥珀色（bg-amber-50 / border-amber-200） |
| `safety` | 红色（bg-red-50 / border-red-200） |
...
└──────────────────────────┬─────────────────────────────────────┘
                           ...
                           ▼
┌────────────────────────────────────────────────────────────────┐
│            总控智能体 ZongkongOrchestrator                      │
│                                                                │
│  意图分类（1次LLM）→ route 事件 → 分派子 Agent                  │
│                                                                │
│  data       → DataAgent（实体卡 + compare_values + 模糊匹配）  │
│  experiment → ExperimentAgent（多候选 + 安全门控 + auto-retry）│
│  literature → LiteratureAgent（完整 RAG 规划 Agent）           │
│  hybrid     → DataAgent → ExperimentAgent（链式，抑制done）    │
│  modify     → ModifyAgent → ExperimentAgent（历史注入）        │
│  safety     → SafetyAgent（配方解析 + check_safety）           │
│                                                                │
│  历史压缩：> 8000 chars → LLM 摘要，保留最近 6 轮              │
└──────┬─────────────────────────────┬──────────────────────────┘
       │                             │
       ▼                             ▼
┌──────────────────────────┐  ┌──────────────────────────────────┐
...
```
{"type": "start"}
{"type": "route", "agent": "数据查询 Agent", "icon": "📊", "intent": "data", "reason": "查询 HMX 单一数值属性 → 数据查询路径"}
...
| `POST` | `/chat/stream` | 流式问答（SSE，经总控智能体路由） |
...
| `POST` | `/extract/file` | 即时数据抽取（同步，不写库）：上传 PDF 或 MinerU JSON，返回实体、属性（含置信度）、表格列表 |
...
评估内容：
- **路由准确性**（R-01～R-15）：意图分类是否正确，覆盖全部 6 种意图及边界情况
- **数据 Agent**（D-01～D-05）：LLM 回答中是否包含预期数值范围内的数字
- **实验 Agent**（E-01～E-03）：回答是否包含配方比例、工艺步骤、安全评估等必要字段
...
│   │       ├── entities.py      # 实体卡片
│   │       ├── graph.py         # 知识图谱浏览
│   │       ├── extract.py       # 即时数据抽取端点（LLM NER + 表格合并 + 标题推断）
│   │       └── health.py
│   ├── core/
...
│       └── zongkong/            # 总控智能体
│           ├── orchestrator.py  # ZongkongOrchestrator（入口，路由 + 历史压缩）
│           ├── intent.py        # 意图分类（IntentType × 6，LLM 单次调用）
│           ├── context.py       # 历史压缩（> 8000 chars → LLM 摘要，保留最近 6 轮）
│           ├── streaming.py     # 流式兜底工具（3段式重试：流式→重试→非流式分块）
│           └── agents/
│               ├── data.py      # DataAgent（实体卡 + 并发compare_values + 模糊匹配）
│               ├── experiment.py # ExperimentAgent（多候选 + 安全门控 + 全拦截auto-retry）
│               ├── literature.py # LiteratureAgent（委托规划 Agent 全流水线）
│               ├── modify.py    # ModifyAgent（提取历史方案 + 增量修改）
│               └── safety.py    # SafetyAgent（配方解析 + check_safety + 流式综合）
├── frontend/                    # PyroPlanner Chat UI（Next.js）
│   ├── app/
│   │   ├── page.tsx             # 主页面：流式问答、route 事件处理、会话管理
│   │   ├── globals.css
│   │   ├── extract/             # 即时数据抽取页（上传 PDF/JSON → LLM NER + 表格）
│   │   ├── experiments/         # 实验历史页（折叠列表 + 懒加载协议方案）
│   │   └── graph/               # 知识图谱页（列表/D3 可视化双模式）
│   ├── components/
│   │   ├── MessageBubble.tsx    # 消息渲染（Agent 徽章 + 6色 intent + Markdown + 计划）
│   │   ├── Sidebar.tsx          # 对话历史侧栏（localStorage）
│   │   ├── PlanBar.tsx          # Agent 工具调用计划展示
│   │   ├── ExperimentPanel.tsx
│   │   ├── ChatChart.tsx        # 对话内图表（Markdown 表格/百分比 → 饼图/柱图/雷达图）
│   │   └── ForceGraph.tsx       # D3 canvas force-directed 知识图谱可视化
│   └── lib/
...
│   ├── zongkong_eval.py               # 总控路由评估脚本（23 题，支持 --mode routing|data|experiment|all）
│   └── zongkong_eval_questions.json   # 23 个总控评估问题（R-01～R-15 路由 + D-01～D-05 数据 + E-01～E-03 实验）
...
| 流式 SSE 输出（start/route/plan/progress/token/done） | ✅ |
...
| 对话历史侧栏（localStorage 含来源，并行后台流，切换对话实时查看进度）| ✅ |
...
| 离线评估模式（CI 友好） | ✅ |
| GitHub Actions CI | ✅ |
| **总控智能体（Zongkong Orchestrator）** | ✅ |
| **意图分类（6 种：data/experiment/literature/hybrid/modify/safety）** | ✅ |
| **route SSE 事件（携带 agent/icon/intent/reason，前端显示彩色徽章）** | ✅ |
| **DataAgent（并发实体卡 + compare_values + 4 步模糊实体匹配）** | ✅ |
| **ExperimentAgent（多候选并行 + 全拦截 auto-retry）** | ✅ |
| **ModifyAgent（从对话历史提取上一次方案 + 增量修改）** | ✅ |
| **SafetyAgent（正则配方解析 + 并发 check_safety + 流式综合）** | ✅ |
| **Hybrid 链式执行（DataAgent → ExperimentAgent，抑制 done 传递）** | ✅ |
| **历史压缩（> 8000 chars LLM 摘要，保留最近 6 轮，失败时尾部截断）** | ✅ |
| **Agent 徽章前端可视化（6 色 intent 配色，路由原因副标题）** | ✅ |
| **总控路由评估（23 题，zongkong_eval.py，--mode routing\|data\|experiment\|all）** | ✅ |
| 知识图谱 D3 可视化（force graph，缩放/拖拽/点击邻居高亮） | ✅ |
| 即时数据抽取 `/extract`（LLM NER，置信度，跨页表格合并，标题推断，CSV/JSON 导出） | ✅ |
| 对话可视化（Markdown 表格/百分比 → 饼图/柱图/雷达图） | ✅ |
| 文献来源卡片（sources SSE 事件，可折叠，点击跳转文档/实体，历史对话持久化） | ✅ |
| 实验历史页（/experiments，懒加载协议，配方/属性/步骤/安全状态） | ✅ |
| 实体对比（雷达图 + 属性表，最多 5 个实体） | ✅ |
| 重新生成按钮（重放上一条用户消息） | ✅ |
| 导航 4 列网格布局（侧边栏 2×4 网格，NavTabs 同步更新） | ✅ |
